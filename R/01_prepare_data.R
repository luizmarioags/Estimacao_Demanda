...
prepare_chicken_demand <- function(raw) {
  ...
  # Preserva a renda original antes de criar o log chamado y.
  df <- df |>
    rename(
      q_raw = q,
      income_real = y
  ...
  # Se PC e PB já existirem na base, usa essas variáveis. Caso contrário, calcula.
  if ("pc" %in% names(df)) {
  ...
  # Remove observações problemáticas para os modelos básicos.
  df <- df |>
