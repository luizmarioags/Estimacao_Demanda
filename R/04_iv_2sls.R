...
run_iv <- function(model_name, zvars, df) {
  ...
  # ivreg: y ~ regressors | exogenous regressors + excluded instruments
  fml <- as.formula(
    paste(
      "qc ~ pf + y + pb | y + pb +",
...
readr::write_csv(iv_table, file.path(TABLE_DIR, "r_iv_results.csv"))
message("2SLS em R concluído. Resultado salvo em output/tables/r_iv_results.csv")
