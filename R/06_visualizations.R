...
if (!file.exists(file.path(TABLE_DIR_R, "r_ols_results.csv")))        source("R/02_ols.R")
if (!file.exists(file.path(TABLE_DIR_R, "r_first_stage_results.csv"))) source("R/03_first_stage.R")
if (!file.exists(file.path(TABLE_DIR_R, "r_iv_results.csv")))          source("R/04_iv_2sls.R")

df <- readRDS(processed_path)

year_var <- if ("year" %in% names(df)) "year" else "time"
fig      <- function(name) file.path(FIG_DIR_R, name)
...
# ------------------------------------------------------------
ols_tbl <- readr::read_csv(file.path(TABLE_DIR_R, "r_ols_results.csv"), show_col_types = FALSE) |>
  filter(term == "pf") |>
  transmute(model = "MQO", beta_p = estimate, conf_low, conf_high)

iv_tbl <- readr::read_csv(file.path(TABLE_DIR_R, "r_iv_results.csv"), show_col_types = FALSE) |>
...
# ------------------------------------------------------------
# 4. F efetiva do weakivtest (Stata) ou F robusta auxiliar (R)
# ------------------------------------------------------------
stata_iv_path <- file.path(TABLE_DIR, "Stata", "stata_iv_results_weakivtest.csv")
if (file.exists(stata_iv_path)) {
  ...
  f_tbl <- readr::read_csv(file.path(TABLE_DIR_R, "r_iv_results.csv"), show_col_types = FALSE) |>
...
message("Gráficos R salvos em ", FIG_DIR_R)
