...
run_first_stage <- function(model_name, zvars, df) {
  ...
  coef_table <- broom::tidy(lmtest::coeftest(fit, vcov. = vc)) |>
...
readr::write_csv(first_stage_table, file.path(TABLE_DIR, "r_first_stage_results.csv"))
message("Primeiros estágios concluídos. Resultado salvo em output/tables/r_first_stage_results.csv")
