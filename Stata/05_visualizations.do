...
graph export "$FIGDIR_STATA/01_series_temporais_variaveis_principais_stata.png", replace width(2400)
...
twoway (scatter qc pf) (lfit qc pf), ///
    ...
    legend(off)
graph export "$FIGDIR_STATA/02_scatter_qc_pf_mqo_stata.png", replace width(2400)

* ------------------------------------------------------------
* 3. Coeficientes beta_p: MQO e 2SLS
* ------------------------------------------------------------
tempfile iv ols
import delimited using "$TABLEDIR_STATA/stata_iv_results_weakivtest.csv", clear
keep model beta_p conf_low conf_high
save `iv', replace

import delimited using "$TABLEDIR_STATA/stata_ols_results.csv", clear
keep if term == "pf"
* FIX: 'model' já existe no CSV (conteúdo "OLS_HC1"); usa replace em vez de gen.
replace model = "MQO"
...
twoway (rcap conf_low conf_high order_model, horizontal) ///
       ...
       legend(off)
graph export "$FIGDIR_STATA/03_coeficientes_beta_p_mqo_2sls_stata.png", replace width(2400)
...
twoway (bar f_eff model_num, barwidth(.65)) ///
       ...
       legend(off)
graph export "$FIGDIR_STATA/04_f_instrumentos_por_modelo_stata.png", replace width(2400)
...
graph export "$FIGDIR_STATA/05_primeiro_estagio_scatter_instrumentos_stata.png", replace width(2400)
...
twoway (scatter resid_ols fitted_ols) (lowess resid_ols fitted_ols), ///
    ...
    legend(off)
graph export "$FIGDIR_STATA/06_residuos_versus_ajustados_mqo_stata.png", replace width(2400)

di as result "Gráficos Stata salvos em $FIGDIR_STATA."
